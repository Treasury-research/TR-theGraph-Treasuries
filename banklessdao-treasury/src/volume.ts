...
export function handlePoolCreated(event: PoolCreated): void {
  if (event.params.token0.equals(BANK) || event.params.token1.equals(BANK)) {
...
export function handleBlock(block: ethereum.Block): void {
  ...
  if (block.timestamp.gt(swap!.timestamp.plus(ONE_DAY))) {
    ...
    let dailyVolume = DailyVolume.load(BANK.toHex());
...
export function handleSwap(event: Swap): void {
  let token = Token.load(BANK.toHex());
  if (!token) return;
  ...
  let dailyVolume = DailyVolume.load(BANK.toHex());
  if (!dailyVolume) {
    dailyVolume = new DailyVolume(BANK.toHex());
    dailyVolume.symbol = "BANK";
  }
