...
export function handlePoolCreated(event: PoolCreated): void {
  if (event.params.token0.equals(Sushi) || event.params.token1.equals(Sushi)) {
...
export function handleBlock(block: ethereum.Block): void {
  ...
  if (block.timestamp.gt(swap!.timestamp.plus(ONE_DAY))) {
    ...
    let dailyVolume = DailyVolume.load(Sushi.toHex());
...
export function handleSwap(event: Swap): void {
  let token = Token.load(Sushi.toHex());
  if (!token) return;
  ...
  let dailyVolume = DailyVolume.load(Sushi.toHex());
  if (!dailyVolume) {
    dailyVolume = new DailyVolume(Sushi.toHex());
    dailyVolume.symbol = "Sushi";
  }
